classdef BEMSolver
    ...
    properties
        ...
        uInf (1,1) double {mustBeNonnegative} = 0.0;    % m/s
        TSR (1,1) double {mustBeNonnegative} = 6.0;     % tip speed ratio
        rRotor (1,1) double {mustBeNonnegative} = 1.0;  % radius of rotor m
        rTipRatio (1,1) double {mustBeNonnegative};     % r_tip/R_rotor
        rRootRatio (1,1) double {mustBeNonnegative};    % r_root/R_rotor
        ...
        bladePitch (1,1) double {mustBeNonnegative} = 45;  % deg
...
%                 lengthSegments = diff(segmentBounds);
        ...
        function [cAx, cAz] = computeLoadsSegment(obj, uRotor, uTan, ...
            ...
            alpha = phi - twistAngle - bladePitch;  % angle of attack
        ...
        function solTotal = solveRotor(obj)
            ...
            solTotal = zeros(obj.nSegments, 4);
            for i = obj.nSegments
                solSegment = solveStreamtube(obj, obj.locSegment(i), ...
                    obj.locSegment(i+1), obj.rRootRatio, obj.rTipRatio, ...
                    obj.rRotor, obj.uInf, obj.Omega, obj.nBlades);
                solTotal(i, :) = solSegment;
            end
        end
        
        function [a, aprime, Az, Ax] = solveStreamtube(obj, rR1, rR2, ...
             ...
             for i = 1:obj.nIter
                 ...
                 % compute new iterant a'
                 aprime = Az/(4*pi*uInf*(1-a)*Omega*(rR*rRotor)^2)/fTot;
